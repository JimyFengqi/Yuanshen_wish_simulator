...
def download_pictures():
                    ...
                    if tb.string is None:
                        if tb.find(
                            ...
                            sub_struct = {'img': tb.find(
                                'img').get("src")}
                        trs = tb.find_all('tr')
                        sub_struct['five'] = [tr.string for tr in trs[3].find_all('a')]
                        sub_struct['four'] = [tr.string for tr in trs[4].find_all('a')]
    ...
    def download_pic():
        ...
        print("Start 获取 info from  https://api-static.mihoyo.com/common/blackboard/ys_obc/v1/home/content/list?app_sn=ys_obc&channel_id=189")
        pmap = pmap['data']['list'][0]['children'][0]['list']
        for Sinfo in pmap:
            print(f"Start 获取{Sinfo['title']}, id:{Sinfo['content_id']}")
            pic = get_pic(Sinfo['content_id'])
            if pic:
                main_struct[Sinfo['title']] = pic
            else:
                print(f"获取{Sinfo['title']}的数据 lost")
    ...
    def get_pic(cid):
        ...
        if text.get('data',{}):
            head = text.get('data',{}).get("content",{}).get("contents",[])[0].get('text',{})
            bs = BeautifulSoup(head, 'lxml')
            stl = bs.find('div', attrs={"data-part": "newMain"}).find('div').find('div')['style'].split('url(\"')
            stl = stl[-1].split('\");')[0]
            return stl
        else:
            print(f'get file failed from https://api-static.mihoyo.com/common/blackboard/ys_obc/v1/content/info?app_sn=ys_obc&content_id={cid}')
